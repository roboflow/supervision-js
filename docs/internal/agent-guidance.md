...
- `src/index.ts` is a minimal package entrypoint. Keep public media renderer
  ...
  presentation styles in `src/styles/`, and Pixi scene composition plus
  renderer implementations in `src/renderers/`.
...
- Use package-private TypeScript aliases for cross-folder imports inside
  ...
  `#styles/...`, `#types/...`, and `#utils/...`. Prefer `#types/...` for
  internal type modules; do not use `@types/...`, which reads like
  DefinitelyTyped package space. Same-folder imports may stay relative when
  that is clearer.
