...
describe("demo presentation", () => {
  ...
  it("lowers the ellipse annotator to a true arc through the shape slot", () => {
    const presentation = createDemoPresentation({
      ...defaultDemoPresentationSettings,
    ...
    expect(instructions?.[0]?.fill).toBeUndefined();
  });

  it("lowers the triangle annotator to an anchored marker above the box", () => {
    const presentation = createDemoPresentation({
      ...defaultDemoPresentationSettings,
      boxAnnotator: DemoBoxAnnotator.Triangle,
      boxTriangleSize: 16,
    });
    const context = {
      detectionIndex: 0,
    ...
    expect(instructions?.[0]?.fill).toMatchObject({ alpha: 1 });
  });

  it("keeps the shape slot empty for box-layer annotators", () => {
    const presentation = createDemoPresentation(
      defaultDemoPresentationSettings,
    );
