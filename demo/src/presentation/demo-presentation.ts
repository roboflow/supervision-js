...
export interface DemoPresentationSettings {
  ...
  readonly boxHaloSpread: number;
  readonly haloEnabled: boolean;
  readonly haloSpread: number;
  ...
  readonly markerIconSize: number;
...
export function createDemoPresentation(
  ...
  return {
    ...
    boxStyle:
      settings.boxesEnabled && settings.boxAnnotator !== DemoBoxAnnotator.Halo
        ? createDemoBoxStyle(settings)
        : null,
    ...
    maskStyle:
      settings.masksEnabled || settings.haloEnabled
        ? createDemoMaskStyle(settings)
        : null,
    ...
    polylineStyle: settings.polylinesEnabled
      ? createDemoPolylineStyle(settings)
      : null,
    shapeStyle: usesShapeSlot(settings) ? createDemoShapeStyle(settings) : null,
  };
}

function usesShapeSlot(settings: DemoPresentationSettings) {
  return (
...
function createEmojiIconHref(emoji: string) {
  return `data:image/svg+xml,${encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 32 32">` +
      `<text x="16" y="25" font-size="24" text-anchor="middle">${emoji}</text>` +
      `</svg>`,
  )}`;
}

const DEMO_CLASS_ICON_HREFS: Record<string, string> = {
  basketball: createEmojiIconHref("\u{1F3C0}"),
  cow: createEmojiIconHref("\u{1F404}"),
  horse: createEmojiIconHref("\u{1F434}"),
  person: createEmojiIconHref("\u{1F9CD}"),
  "white team player": createEmojiIconHref("\u{26AA}"),
  "yellow team player": createEmojiIconHref("\u{1F7E1}"),
};

const DEMO_DEFAULT_ICON_HREF = createEmojiIconHref("\u{1F4CD}");

// Layered strokes approximating a blurred glow; a shader-driven halo waits
// on the roadmap's media-effect primitives. Ring alphas are increments of a
// quadratic falloff so the stacked strokes integrate into a smooth fade
// instead of visible bands.
const HALO_RING_COUNT = 10;
const HALO_CORE_ALPHA = 0.85;
const HALO_GLOW_ALPHA = 0.5;

function createContourHaloInstructions(
  ...
  for (let ring = HALO_RING_COUNT - 1; ring >= 0; ring -= 1) {
    const inner = ring / HALO_RING_COUNT;
    const outer = (ring + 1) / HALO_RING_COUNT;
    const alpha = HALO_GLOW_ALPHA * ((1 - inner) ** 2 - (1 - outer) ** 2);
    ...
    instructions.push({
      ...
      stroke: {
        alpha,
        color,
        width: baseWidth + spread * outer,
      },
    });
  }

  // Crisp core outline over the glow so the contour stays defined.
  instructions.push({
    closed: true,
    kind: ShapeInstructionKind.Path,
    segments: [points],
    stroke: { alpha: HALO_CORE_ALPHA, color, width: baseWidth },
  });

  return instructions;
}

function rectContourPoints(rect: Rect) {
  ...
  return {
    resolve(
      ...
      if (
        ...
        settings.boxAnnotator === DemoBoxAnnotator.Halo
      ) {
        instructions.push(
          ...createContourHaloInstructions(
            rectContourPoints(rect),
            style.stroke,
            settings.boxStrokeWidth,
            settings.boxHaloSpread,
          ),
        );
      }
...
function resolveDemoMarkerInstructions(
  detection: Detection,
  rect: Rect,
  ...
  switch (settings.markerAnnotator) {
    ...
    case DemoMarkerAnnotator.Icon: {
      // Anchored above the box like the triangle marker.
      const mediaSize = settings.markerIconSize / scale;

      return [
        {
          href:
            (detection.className &&
              DEMO_CLASS_ICON_HREFS[detection.className]) ||
            DEMO_DEFAULT_ICON_HREF,
          kind: ShapeInstructionKind.Icon,
          point: { x: rect.x, y: rect.y - rect.height / 2 - mediaSize / 2 },
          size: settings.markerIconSize,
          sizeSpace: MarkerSizeSpace.Screen,
        },
      ];
    }
...
function createDemoMaskStyle(settings: DemoPresentationSettings): MaskStyle {
  return {
    ...
    resolve(detection: Detection): MaskDrawInstruction | undefined {
      ...
      const style = resolveClassStyle(detection, settings);
      // The GPU mask halo follows the exact mask silhouette; with masks
      // disabled the fill and border stay invisible and only the halo shows.
      const halo = settings.haloEnabled
        ? {
            alpha: 0.65,
            color: style.fill,
