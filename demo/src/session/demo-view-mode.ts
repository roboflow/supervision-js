 ...
 */
export const DEMO_VIEW_MODE_STORAGE_KEY = "supervision-demo:view-mode";
...
export function readStoredDemoViewMode(fallback: DemoViewMode): DemoViewMode {
  const stored = readStorage()?.getItem(DEMO_VIEW_MODE_STORAGE_KEY);
...
export function writeStoredDemoViewMode(mode: DemoViewMode): void {
  readStorage()?.setItem(DEMO_VIEW_MODE_STORAGE_KEY, mode);
}
